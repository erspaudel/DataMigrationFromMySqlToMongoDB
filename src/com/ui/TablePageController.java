...
import com.multithreading.MySQLToFileThreadInner;
...
public class TablePageController implements Initializable {
	...
	public void processConfiguration() throws Exception {
		...
		CountDownLatch latch = new CountDownLatch(MySqlUtils.getTotalThreads(ConfigUtils.getDbName()));
...
//			if (!StringUtils.equalsIgnoreCase("college", tableName)){
//				continue;
//			}
			...
			for (int i = 1; i <= iteratorLength; i++) {
				...
				Document doc = tableDocument;

				MySQLToFileThreadInner inner = new MySQLToFileThreadInner(doc, latch, startLimit, endLimit, i);
		...
		if (TestUtils.MIGRATE_TO_MONGODB) {

			executor = Executors.newFixedThreadPool(ConfigUtils.FIXED_THREAD_POOL);
	...
	public static void main(String[] args) throws Exception {
		new TablePageController().processConfiguration();
	}
}
