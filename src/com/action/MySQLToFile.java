...
public class MySQLToFile {
	...
	public void processTable(Document tableDocument, int startLimit, int endLimit, int iteratorCount) throws Exception {

		String tableName = tableDocument.getString(ConfigUtils.TABLE_NAME);

		try {

			debug(tableName.toUpperCase() + "- " + iteratorCount + ": " + startLimit + ", " + endLimit);

			long startTime = System.currentTimeMillis();

			List<Document> documents = getDocuments(tableDocument, startLimit, endLimit, iteratorCount);

			long endTime = System.currentTimeMillis();

			debug(tableName.toUpperCase() + "- " + iteratorCount + ": " + startLimit + ", " + endLimit
					+ ", Total Documents: " + documents.size() + ", Time Taken: " + (endTime - startTime));

			FileExporter exporter = new FileExporter(tableName, iteratorCount);
	...
	public List<Document> getDocuments(Document tableDocument, int startLimit, int endLimit, int iterator)
			throws Exception {
		...
		try {
			...
			for (int i = startLimit; i < endLimit; i++) {
				...
				// System.err.println(tableName.toUpperCase() + "-" + iterator +
				// ": " + i);

				Document document = getDocumentFromMySql(tableDocument, i, orderBy, primaryKeys, lastId);
				
				

//				if (StringUtils.equalsIgnoreCase("class_student", tableName)) {
//					 System.out.println("\t\t\t" + document.toJson());
//				}

				lastId = getIdFromDocument(document);
				...
				convertForeignIdsIntoMongoDBObjectId(document, tableName);

				removeForeignKeys(tableName, document);
	...
	private void getOneToOneDocumentRecursive(String tableName, Document document, String fkTableName, boolean embed,
		...
		// System.out.println("EMBED One-to-one (" + tableName + ") :" +
		// embedCounter);
		...
		// System.out.println(tableName + ", " + fkTableName +", "+embedCounter+
		// ", " + document.toJson() + ", " + sb.toString());

		ResultSet rs = MySqlUtils.getConnection().createStatement().executeQuery(sb.toString());
			...
			for (int k = 1; k <= meta.getColumnCount(); k++) {
				...
				if (embed) {
					if (StringUtils.isNotEmpty(value)) {
						...
						boolean isForeignKey = MySqlUtils.isForeignKey(tableName, key);

//						System.out.println(tableName + ", " + fkTableName + ", " + key + ": " + isForeignKey);
						...
						// MongoUtils.getObjectId(id).toString());
						// }
			...
			if (embed && document2 != null) {
				convertForeignIdsIntoMongoDBObjectId(document2, fkTableName);
	...
	public void processOneToFew(String tableName, Document document, Document relationDocument) throws SQLException {
		...
		getOneToFewDocumentRecursive(tableName, document, fkTableName, embedAll, true, DEFAULT_EMBED_COUNTER);

		// System.err.println(tableName+", "+fkTableName);
		//
		// List<String> fks = MySqlUtils.getStaticForeignKeys(tableName);
		//
		// for (Map.Entry<String, Object> map : document.entrySet()) {
		//
		// String orgTableName = map.getKey().substring(0, map.getKey().length()
		// - 1);
		//
		// System.out.println("ORG: "+orgTableName);
		//
		// if (fks.contains(orgTableName)) {
		// List<Document> innerDocs = (List<Document>)
		// document.get(map.getKey());
		//
		// System.out.println("inner docs: "+innerDocs.size());
		//
		// for (Document doc : innerDocs) {
		// getOneToOneDocumentRecursive(tableName, doc, orgTableName, true,
	...
	private void getOneToFewDocumentRecursive(String tableName, Document document, String fkTableName, boolean embedAll,
		...
		// System.out.println("EMBED one-to-few (" + tableName + ") :" +
		// embedCounter);
...
//		System.out.println(tableName + ", " + fkTableName + ": " + sb.toString());
...
//						System.out.println(embedCounter + ", " + fkTableName + ", " + key + ", "
//								+ MySqlUtils.isForeignKey(fkTableName, key));
						...
						if (embedCounter < ConfigUtils.getEmbedDocumentLimit()
							...
							// System.err.println(tableName.toUpperCase() + "
							// o2f: " + embedCounter);
							getOneToOneDocumentRecursive(fkTableName, doc, primaryTableName, true, ++embedCounter);
						}
					}
	...
	public void processOneToMany(String tableName, Document document, Document relationDocument) throws SQLException {
		...
		if (StringUtils.equalsIgnoreCase(thumbRuleCode, ConfigUtils.RELATION_CODE_One_to_Many_3)) {
			document.append(WordUtils.getPlular(fkTableName), documents);
		}
		try {
			...
			if (StringUtils.equalsIgnoreCase(thumbRuleCode, ConfigUtils.RELATION_CODE_One_to_Many_4)
					|| isAutomatedProcess) {
				if (ids.size() > 0) {
					document.append(WordUtils.getPlular(fkTableName), ids);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void getOneToManyDocumentRecursive(String tableName, Document document, String fkTableName, boolean embed,
		...
		// System.out.println("EMBED One-to-Many (" + tableName + ") :" +
		// embedCounter);
	...
	public void processOneToSquillion(String tableName, Document document, Document relationDocument)
		...
		while (rs.next()) {
			...
			for (int k = 1; k <= meta.getColumnCount(); k++) {
				...
				if (StringUtils.isNotEmpty(value) && !primaryKeys.contains(key)) {
					document.append(key, value);
				}
	...
	public Document getDocumentFromMySql(Document tableDocument, int startLimit, String orderBy,
			List<String> primaryKeys, String lastId) throws SQLException {
	...
	public Document getDocumentByQuery(String tableName, String query, boolean includeMongoDBId,
			List<String> primaryKeys) throws SQLException {

		Document document = new Document();

		ResultSet rs = MySqlUtils.getConnection().createStatement().executeQuery(query.toString());
	...
	public void convertForeignIdsIntoMongoDBObjectId(Document document, String tableName) throws SQLException {
		...
		for (String fk : fks) {
			...
			String value = document.getString(fk);
			...
			if (StringUtils.isNotEmpty(value)) {
				...
				if (isMongoDbObjectId(value)) {
					...
					if (StringUtils.isNumeric(value)) {
						int mySqlId = Integer.parseInt(value);
						document.append(fk, MongoUtils.getObjectId(mySqlId).toString());
					}
				}
			}

			String pkTable = MySqlUtils.getPrimaryTableName(tableName, fk);

			if (document.containsKey(pkTable)) {
				document.remove(fk);
			}
		}
		...
		for (String pk : primaryKeys) {
			...
			String value = document.getString(pk);

			if (document.containsKey(pk) && !isMongoDbObjectId(value)) {
				document.remove(pk);
			}
		}
	...
	public boolean isMongoDbObjectId(String value) {
		...
		for (Map.Entry<String, Object> value : document.entrySet()) {
			...
			for (String key : keys) {
				...
				if (StringUtils.equalsIgnoreCase(pkId, value.getKey())) {
					...
					if (isRecursive) {
						where += key + "=" + value.getValue().toString();
					} else {
	...
	public void removeForeignKeys(String tableName, Document document) {

		Object object = document.get(ConfigUtils.RELATIONS);
		...
		for (Map.Entry<String, Object> value : document.entrySet()) {
			...
			if (fks.contains(value.getKey())) {

				if (StringUtils.isNumeric(value.getValue().toString())) {
					toBeRemoved.add(value.getKey());
				}
			}
		}
	...
	public String getIdFromDocument(Document document) {
